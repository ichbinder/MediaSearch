    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (userId === adminUser.id) {
          return res.status(400).json({
            error:
              "Benutzen Sie die Profilseite um Ihr eigenes Passwort zu ändern",
          });
        }
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        if (userId === adminUser.id) {
          return res.status(400).json({
            error: "Sie können Ihren eigenen Status nicht ändern",
          });
        }
        ...
        const updatedUser = await db
          .update(users)
          .set({ is_approved: !user.is_approved })
          .where(eq(users.id, userId))
          .returning();
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (userId === adminUser.id) {
          return res.status(400).json({
            error: "Sie können Ihren eigenen Benutzernamen nicht ändern",
          });
        }
